      ...
      <template slot="document" slot-scope="{ item }">
        <div>
          <a
            ...
            @click.stop="openLinkInNewTab(item.document[item.document.length - 1].link)"
            >Download</a
          >
        </div>
      </template>
...
export default {
  ...
  methods: {
    ...
    updateDocument(payload) {
      if (payload) {
        this.documentStatus = payload.status.id
    ...
    async performAction() {
      if (this.documentStatus === 'new' || !this.documentReview) {
