<template>
  <nav v-if="isAuthenticated" class="lg:h-full flex flex-col w-full">
    <div class="nav-inner flex flex-row lg:flex-col items-center w-full">
      ...
      <div v-if="isAuthenticated" class="text-center w-full ml-auto lg:ml-0 user-details">
        <div
          v-if="currentUser && currentUser.photo_url"
          class="rounded-full h-24 w-24 m-auto"
          :style="{ backgroundImage: 'url(' + currentUser.photo_url + ')' }"
        />
        <div class="p-2 font-bold text-center text-white w-full text-sm lg:text-base flex flex-row flex-wrap items-center justify-around">
          <nuxt-link :to="'/user/settings'" class="">
            <span>{{ currentUser.username | truncate(8) }}</span>
          </nuxt-link>
          <span>|</span>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters('UserPaymentsStore', ['getUserWallet', 'getWalletBalance']),
  },
  filters: {
    truncate(string, value) {
      const stringLength = Array.from(string)
      if (stringLength.length > value) {
