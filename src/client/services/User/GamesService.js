...
export default {
  ...
  getLeaderboardGames() {
    return AxiosService.get(`${basePath}?status=live&include=players,map`)
  },
  getInvitationGames() {
  ...
  getUserEarningsGames() {
    return AxiosService.get(
      `${basePath}?status=finished&player_status=all&include=map,players,historical_earnings,historical_earnings.user,historical_earnings.game_player`
    )
  },
  getUserFreeCredits() {
    return AxiosService.get(`${basePath}/free-credits`)
  },
}
