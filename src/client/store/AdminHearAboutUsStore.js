...
export const getters = {
  ...
  platformsWithCount: (state) => state.platformsWithCount,
...
export const mutations = {
  ...
  [CONST.GET_HAU_PLATFORM_COUNT]: (state) => {},
  [CONST.SET_HAU_PLATFORM_COUNT]: (state, payload) => {
    state.platformsWithCount = payload
  },
  [CONST.ERROR_HAU_PLATFORM_COUNT]: (state, payload) => {
    state.errorMsg = payload
  },
}
...
export const actions = {
  ...
  getAllPlatformsWithCountHAU({ commit }) {
    commit(CONST.GET_HAU_PLATFORM_COUNT)
    try {
      return new Promise((resolve, reject) => {
        HearAboutUs.getHearAboutUsWithCount()
          .then((response) => {
            commit(CONST.SET_HAU_PLATFORM_COUNT, response.data.data)
            resolve(response.data.data)
          })
          .catch((error) => {
            commit(CONST.ERROR_HAU_PLATFORM_COUNT, error)

            reject(error)
          })
      })
    } catch (e) {
      commit(CONST.ERROR_HAU_PLATFORM_COUNT, e)
    }
  },
