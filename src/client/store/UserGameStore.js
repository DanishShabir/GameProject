...
export const getters = {
  ...
  freeCredits: (state) => state.freeCredits,
}
...
export const mutations = {
  ...
  // Free Credits
  [CONST.START_USER_FREE_CREDITS]: (state) => {},
  [CONST.FINISH_USER_FREE_CREDITS]: (state, payload) => {
    state.freeCredits = payload
  },
}
...
export const actions = {
  ...
  getUserFreeCredits({ commit }) {
    commit(CONST.START_USER_FREE_CREDITS)
    try {
      return new Promise((resolve, reject) => {
        GamesService.getUserFreeCredits()
          .then((response) => {
            const data = response.data.data
            commit(CONST.FINISH_USER_FREE_CREDITS, data)
            resolve(data)
          })
          .catch((error) => {
            commit(CONST.ERROR_USER_FREE_CREDITS, error)

            reject(error)
          })
      })
    } catch (e) {
      commit(CONST.ERROR_USER_FREE_CREDITS, e)
    }
  },
}
