      ...
      return new Promise((resolve, reject) => {
        PaymentService.getWallet()
          .then((response) => {
            ...
            commit('SET_WALLET_BALANCE', response.data.data[0].balance)
            resolve(response)
          })
          .catch((error) => {
        ...
        PaymentService.getPurchaseHistory(walletId)
          .then((response) => {
            const data = response.data.data.map((purchase) => ({
              ...
              status: purchase.transaction ? purchase.transaction.status.name : '',
              paymentMethod: purchase.transaction ? purchase.transaction.paymentMethod : '',
              transactional_type: purchase.transaction ? purchase.transaction.transactional_type : '',
    ...
    try {
      return new Promise((resolve, reject) => {
        PaymentService.createDeposit(payload)
          ...
          .catch((error) => {
            commit(CONST.ERROR_CREATE_USER_DEPOSIT, error)
