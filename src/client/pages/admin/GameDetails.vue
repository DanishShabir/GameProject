<template>
  <div>
      ...
      >
        ...
        <div class="card-body p-4 grid grid-cols-3 gap-2">
          <div class="mb-2 text-grey flex flex-col">
            ...
            <span>{{ $moment(currentGameDetails.start_date).format('DD/MM/YYYY hh:mm a') }}</span>
          </div>
          <div class="mb-2 text-grey flex flex-col">
            <b class="text-green-500 mb-1">End date</b>
            <span>{{ $moment(currentGameDetails.end_date).format('DD/MM/YYYY hh:mm a') }}</span>
          </div>
          <!-- <div class="mb-2 text-grey flex flex-col">
            <b class="text-green-500 mb-1">Admin fee percentage</b>
            <span>{{ currentGameDetails.admin_fee_percentage + "%" }}</span>
          </div> -->
          <!-- <div class="mb-2 text-grey flex flex-col">
    ...
    <div class="grid w-3/4 gap-4 mx-auto grid-cols-2">
      <div v-if="currentGameDetails.status.id === 'waiting-for-pay-out'" class="flex items-center flex-row my-8 justify-center gap-4 bg">
        <button class="card-header p-3 rounded text-2xl text-center text-white bg-green-700 focus:outline-none h-a w-full" @click="reviewGame()">Review</button>
      </div>
      <div v-if="isEditButtonVisible" class="flex items-center flex-row my-8 justify-center gap-4 bg">
        <button class="card-header p-3 rounded text-2xl text-center text-white bg-green-700 focus:outline-none h-a w-full" @click="editGame()">
          Edit Game
        </button>
      </div>
      <div v-if="isDeleteButtonVisible" class="flex items-center flex-row my-8 justify-center gap-4 bg">
        <button class="card-header p-3 rounded text-2xl text-center text-white bg-green-700 focus:outline-none h-a w-full" @click="deleteGame()">
          Delete Game
        </button>
      </div>
    </div>
    ...
    <base-modal :showing="modalVisible" @close="modalVisible = false">
      ...
      <template slot="content">
        <edit-game-form @edit-game="getDetails('edit')" />
      </template>
    </base-modal>
    <base-modal :showing="reviewModalVisible" @close="reviewModalVisible = false">
      <template slot="title">
        <h1 class="text-white text-2xl mb-4">
          {{ $t('review_game') }}
        </h1>
      ...
      <template slot="content">
        <review-game-form @review-game="getDetails('review')" />
      </template>
    </base-modal>
  </div>
</template>
...
import reviewGameForm from '~/components/Admin/reviewGameForm.vue'
...
export default {
  ...
  computed: {
    ...
    isEditButtonVisible() {
      return this.currentGameDetails.status.id !== 'waiting-for-pay-out' && this.currentGameDetails.status.id !== 'finished'
    },
    isDeleteButtonVisible() {
      return this.currentGameDetails.status.id !== 'finished'
    },
    mappedGames() {
      if (!this.currentGameDetails.players && !this.currentGameDetails.players.length) {
        return []
      }
      let games = []
      const gameObject = {
        gameName: this.currentGameDetails.name, 
        players: this.currentGameDetails.players
      }
  ...
  methods: {
    ...
    async getDetails(type) {
      ...
      const text = type === 'edit' ? this.$t('edit_successfull') : this.$t('review_successfull')
      swal.fire({
        text,
        type: 'success',
        title: type === 'edit' ? 'Edited Successfully' : 'Reviewed Successfully',
