<template>
  <div v-if="!isLoading" class="px-4 py-4">
    <div v-if="getUserKYCValidation[0].status.name !== 'Approved'">
      ...
      <div>
        <h2 class="text-white invite-text text-center">
          Before any winnings can be paid, you must verify yourself. To speed up the process, you can upload a government issued photo ID below.
        </h2>
      </div>
      <div class="container mx-auto mt-10">
        <div class="grid grid-cols-1 gap-4">
          <div>
            <base-table :header-items="headerItems" :records="getUserKYCValidation[0].document" :paginate="false" class="mb-2">
              <template slot="position" slot-scope="{ index }">
                <div>{{ index + 1 }}</div>
              </template>
              <template slot="deleted_at" slot-scope="{ item }">
                <div>{{ item.deleted_at ? 'Rejected' : 'Pending' }}</div>
              </template>
            </base-table>
          </div>
        </div>
    ...
    <div v-else class="grid grid-cols-1 gap-4 max-w-3xl mx-auto">
      <div v-if="getUserKYCValidation[0].status.name === 'Approved'" class="game-card flex flex-col relative items-center p-8">
        <h1 class="text-white text-2xl mb-4 text-center mt-4">KYC</h1>
        <p class="mb-4">Your KYC approved on {{ $moment(getUserKYCValidation[0].reviewed_at).format('DD/MM/YYYY') }}</p>
      </div>
    </div>
    <!-- <div v-else class="grid grid-cols-1 gap-4 max-w-3xl mx-auto">
      <div v-if="getUserDocument" class="game-card flex flex-col relative items-center p-8">
        <h1 class="text-white text-2xl mb-4 text-center mt-4">KYC</h1>
        <p class="mb-4">You have uploaded the document below.</p>
        <div class="game-card-body p-4 border-red border-t-4 w-full rounded justify-center sm:w-auto">
          <span class="flex flex-row items-center mt-2">
            <div class="block mr-2 w-full">
              ...
              <div class="flex mb-2 flex-col lg:flex-row">
                ...
                <span class="text-white">{{ getUserKYCValidation[0].status.name === 'New' ? 'Pending' : getUserKYCValidation[0].status.name }}</span>
              </div>
              <div>
              ...
              <div class="flex items-center flex-row mt-4 justify-center gap-4">
                <button class="btn p-3 text-center bg-gold focus:outline-none h-a w-full" @click="editKYC()">Edit</button>
                <button class="btn p-3 text-center bg-gold focus:outline-none h-a w-full" @click="deleteKYV()">Delete</button>
              </div>
            </div>
          </span>
        </div>
    ...
    <base-modal :showing="modalVisible" @close="modalVisible = false">
      ...
      <template slot="content">
        <div class="max-w-xl flex flex-col w-full" @keyup.enter="onFileChange">
          <div class="group">
            <label class="text-white text-xs group-focus:text-red-600 transition-colors duration-300"> Document </label>
            <input
              ref="file"
              type="file"
              name="file"
          ...
          <div class="group">
            <label class="text-white text-xs group-focus:text-red-600 transition-colors duration-300"> Type ( pdf, png, jpg, jpeg ) </label>
            <select v-model="documentType" name="documentType" class="w-full bg-white text-gray-900 border-r-2 p-1">
          ...
          <button :class="['btn mt-5 w-full', { disabled: loading }]" type="button" :disabled="loading" @click="onFileChange()">
            <span v-if="loading">
              Uploading document
  ...
  methods: {
    ...
    async getKycValidationDocs() {
      ...
      if (this.getUserKYCValidation.length) {
        this.documents = this.getUserKYCValidation[0].documents
        const validations = this.getUserKYCValidation[0]
        this.kycId = validations.id
        // this.documentId = validations.document && validations.document.id ? validations.document.id : null
        // if (!this.kycId) {
      ...
      if (this.document && this.isValidFile(this.document)) {
        ...
        if (!this.isEdit) {
          try {
            ...
            this.documentId = this.uploadUserKYCDocument && this.uploadUserKYCDocument.id ? this.uploadUserKYCDocument.id : null
