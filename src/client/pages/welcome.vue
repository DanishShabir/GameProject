<template>
  <div>
    <section class="c-hero">
      ...
      <video ref="videoRef" class="c-hero__video" autoplay muted playsinline loop>
        <source src="https://player.vimeo.com/external/578015777.sd.mp4?s=34492cb4989ba18e03fc60d1cdbb40d295734dab&profile_id=165" type="video/mp4" />
      </video>
    </section>
    <section class="text-center pt-8 lg:pt-16">
      ...
      <h2 class="text-white text-2xl lg:text-3xl">Why leave things to chance?</h2>
    </section>
    <section class="py-8 lg:py-16">
      <div class="max-w-4xl m-auto">
        <div
          ...
          :class="liveGames.length < 2 ? 'md:grid-cols-1 max-w-lg mx-auto' : 'md:grid-cols-2'"
        >
          <div v-for="(game, index) in liveGames" :key="index" class="game-card flex flex-col max-w-2xl relative">
            <div v-if="authenticated">
              <nuxt-link :to="setUrl(balance, game)">
                <div
                ...
                <div class="game-card-body p-4 border-red border-t-4 flex-1">
                  ...
                  <span class="text-white">Choose your launch point, aim, then fire the balls to destroy the blocks.</span>
                  <span class="flex flex-row items-center mt-2">
                    <div class="block mr-2 w-full">
                      <div class="flex mb-2">
                        <span class="text-gold font-semibold mr-2">Jackpot:</span>
                        <span class="text-white">£{{ game.jackpot_value }} Guaranteed</span>
                      </div>
                      ...
                      <div class="flex mb-2 flex-col md:flex-row">
                        ...
                        <span class="text-white">{{ $moment(game.end_date).format('DD/MM/YYYY | hh:mm a') }} GMT</span>
                      </div>
                      <div class="flex mb-2 flex-col md:flex-row">
                        <span class="text-gold font-semibold mr-2">Competition closes in: </span>
                        <client-only>
                          <vac :end-time="new Date(game.end_date).getTime()">
                            <span slot="process" slot-scope="{ timeObj }" class="text-white">{{
                ...
                <div v-if="!isUserHasCredit(game)" class="nocredit-overlay flex justify-center items-center">
                  <h2 class="text-white font-bold text-2xl text-center">Please purchase credits to enter competitions.</h2>
                </div>
              </nuxt-link>
            </div>
            <div v-else>
    ...
    <section class="py-8 lg:py-16 bg-grey">
      <div class="container mx-auto max-w-2xl px-4">
        <div class="grid grid-cols-1 gap-4">
          <div>
            <h1 class="text-2xl lg:text-3xl text-white text-center block mb-4 lg:mb-8">Previous Winners</h1>
          </div>
    ...
    <section class="py-8 lg:py-16">
      <div class="container mx-auto max-w-2xl px-4">
        <div class="grid grid-cols-1 gap-4">
          <div>
            <h1 class="text-2xl lg:text-3xl text-white text-center block">How to play</h1>
          </div>
          <client-only>
            <video id="my-player" class="how-to-video" controls preload="auto" :poster="require('~/assets/images/game-2.png')" data-setup="{}">
              <source src="https://player.vimeo.com/external/578016862.sd.mp4?s=ffc1fc484d12d61ce234305283518e96a93f6943&profile_id=165" type="video/mp4" />
              <p class="vjs-no-js">
...
export default {
  ...
  methods: {
    ...
    isUserHasCredit(game) {
      if (this.gameWallets) {
        let gameWallet = this.gameWallets.filter((gameWallet) => gameWallet.game.id === game.id)
