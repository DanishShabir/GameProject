    ...
    public function handle()
    {
        ...
        try {
            ...
            foreach ($games as $game) {
                ...
                if ($game->game_ext_days && $game->is_deadline_reached &&
                    $game->extensions_count < MAX_GAME_EXTENSION_LIMIT &&
                    $game->pot_value < $game->jackpot_value
                ) {
                    $extensionStartDate = $game->end_date->toDateTimeString();
                    $extensionEndDate = $game->end_date->addDays($game->game_ext_days)->toDateTimeString();
