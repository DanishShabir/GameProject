...
class GamePlayerService extends BaseService
{
    ...
    {
        if (!$game) {
            throw new ErrorException('exception.game_not_exist');
        }
        ...
        if ($game->end_date->diffInMinutes(\Carbon\Carbon::now()) <= 1) {
            Log::error(__METHOD__ . " -- user: " . auth()->user()->email . " -- Game for which the user request to play is about to end in 1 minute", $game->toArray());
            throw new ErrorException('exception.game_is_about_to_end');
        }
        ...
        $map['data']['entry_fee'] = $game->entry_fee;
        ...
        if ($user->isPlayingGameByGameID($game->id)) {
            ...
            if ($isUserPlayingGame && $isUserPlayingGame->pivot) {
                $map['data']['times_played'] = GamePlayerAttemptScore::where([
                    'user_id' => $user->id,
                    'game_id' => $game->id,
                    'play_id' => $isUserPlayingGame->pivot->number_of_times_played
                ])->count();

                return [
                    'game_player_token' => $isUserPlayingGame->pivot->token,
                    'map_data' => $map
                ];
            }
        }
    ...
    private function createGameWallet(User $user, Game $game) : void
    {
        ...
        if ($invitationCodeSender && $invitationCodeSender->game_id === $game->id) {
