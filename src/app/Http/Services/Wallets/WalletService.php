...
class WalletService
{
    ...
    public function createGameTransaction(Game $game, User $user) : Model
    {
        ...
        if ($wallet->balance < $game->entry_fee) {
            Log::error(__METHOD__ . " -- SECURITY:: User_id: ". $user->id ." tried to load a game directly with no credit");
            Log::error(__METHOD__ . " -- user: " . $user->email . " -- User not have an enough money to play the game");
            throw new ErrorException('exception.insufficient_balance_to_play_game', [], 402);
        }
