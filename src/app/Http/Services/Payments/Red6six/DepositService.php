...
class DepositService extends BaseService
{
    ...
    public function getAll(array $data) : Collection
    {
        ...
        return $deposit->orderByDesc('id')->get();
    }
...
//        $user = User::where('id', $data['user_id'])->first();
        $user = RequestPaymentLink::where('link_id', $data['payment_transaction_id'])->first()->user;
        RequestPaymentLink::where('link_id', $data['payment_transaction_id'])->delete();
        ...
        try {
            ...
            $walletTransaction = $userTransaction->walletTransactions()->create([
                'user_id' => $user->id,
                'wallet_id' => $wallet->id,
            ...
            Log::info(__METHOD__ . " -- user: " . $user->email . " -- User wallet Transaction:", $walletTransaction->toArray());
     ...
     * @throws ErrorException
     */
    public function paymentRequestLink(array $data)
    {
        Log::info(__METHOD__ . " -- User deposit:", $data);
        ...
        $accountId = config('services.paytriot.accountId');

        $paytriot = new Merchant_Api(
            config('services.paytriot.serviceUrl'),
            config('services.paytriot.key'),
            config('services.paytriot.secret'),
            'true',
            config('services.paytriot.desKey')
        );

        $data = $paytriot->createPaymentRequestLink(
            'creditcard',
            $data['amount'],
            $accountId,
            $data['currency'],
            $data['location'] . '?status=success',
            $data['location'] . '?status=error',
            route('paytriot.webhook'),
            route('paytriot.webhook'),
            0,
            '2');

        Log::info(__METHOD__ . " -- Payment request link :", $data);

        $user = auth()->user();

        $user->links()->create([
            'user_id' => $user->id,
            'link_id' => $data['payment_request_link'],
        ]);
