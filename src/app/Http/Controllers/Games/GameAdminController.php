...
class GameAdminController extends Controller
{
    ...
    public function updateWinners(Game $game, Request $request)
    {
        try {
            DB::beginTransaction();
            $game = $this->service->updateWinners($game, $request->all());
            DB::commit();
        } catch (ErrorException $e) {
            Log::error(__METHOD__ . " -- Internal error occur in game review");
            DB::rollback();
            $errorCode = Response::HTTP_BAD_REQUEST;
            return $this->failure($errorCode, trans($e->getName(), $e->getParams()), $errorCode);
        }

        return $this->success([], null, trans('messages.success'));
    }
}
