...
class GamePlayerController extends Controller
{
     ...
     */
    public function playGame(int $gameId) : JsonResponse
    {
        ...
        try {
            ...
            if ($e->getStatusCode() === 402) {
                $errorCode = Response::HTTP_PAYMENT_REQUIRED;
            } else {
                $errorCode = Response::HTTP_FORBIDDEN;
            }
            return $this->failure($errorCode, trans($e->getName(), $e->getParams()), $errorCode);
        }
    ...
    public function invitationLink(Game $game) : JsonResponse
    {
        ...
        $gamePlayer = auth()->user()->games()->where('game_id', $game->id)->first();
        $noOfTimesPlayed = $gamePlayer ? $gamePlayer->pivot->number_of_times_played : null;

        return $this->success(['invitation_link' => $invitationLink, 'noOfTimesPlayed' => $noOfTimesPlayed]);
    }
}
